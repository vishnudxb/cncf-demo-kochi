...
openssl req -newkey rsa:2048 -nodes -keyout $TLS_KEY -x509 -days 365 -out $TLS_CERT \
  ...
  -addext "subjectAltName=DNS:vault-0.vault-internal.svc.cluster.local,DNS:vault-1.vault-internal.svc.cluster.local,DNS:vault-2.vault-internal.svc.cluster.local,DNS:vault-0.vault.svc.cluster.local,DNS:vault-1.vault.svc.cluster.local,DNS:vault-2.vault.svc.cluster.local,DNS:vault.vault.svc.cluster.local,DNS:localhost,IP:127.0.0.1"
...
for pod in "${VAULT_PODS[@]}"; do
  ...
  for key in "${UNSEAL_KEYS[@]}"; do
    ...
    sleep 10  # Optional delay between unseal attempts
  done
done
